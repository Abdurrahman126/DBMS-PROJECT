...
create table members(member_id int primary key,member_name varchar(30), member_user_id int ,foreign key(member_user_id) references users(user_id),appointed_as varchar(20) not null);
...
create table participants(participant_id int primary key,user_id int, foreign key(user_id) references users(user_id), competition_id int, foreign key(competition_id) references society_events(event_id), paricipant_name varchar(50));
create table mentor(mentor_id int primary key, mentor_user_id int, foreign key(mentor_user_id) references users(user_id),mentor_name varchar(30),mentor_email varchar(50));
create table certifications(certification_id int primary key, participant_id int, foreign key(participant_id) references participants(participant_id), person_name varchar(30) );
