...
create table members(member_id int primary key,member_user_id int ,foreign key(member_user_id) references users(user_id),appointed_as varchar(20) not null);
...
create table participants(participant_id int primary key,user_id int, foreign key(user_id) references users(user_id), competition_id int, foreign key(competition_id) references society_events(event_id));
